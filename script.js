...
function renderProducts() {
    // Сначала пробуем найти mainSlider (новый ID для главной страницы)
    let grid = document.getElementById('mainSlider');
    
    // Если mainSlider не найден, используем catalogGrid (для обратной совместимости)
    if (!grid) {
...
if (accordionHeaders.length) {
    accordionHeaders.forEach(header => {
        // Находим стрелку внутри заголовка
        const arrow = header.querySelector('.footer__accordion-arrow');
        // Находим ссылку внутри заголовка
        const link = header.querySelector('.footer__accordion-title');
        
        // Клик по стрелке — раскрываем/закрываем подменю
        if (arrow) {
            arrow.addEventListener('click', function(e) {
                e.stopPropagation(); // Останавливаем всплытие
                e.preventDefault(); // Предотвращаем переход по ссылке
                const accordion = this.closest('.footer__accordion');
                if (accordion) {
        ...
        // Клик по заголовку (ссылке) — переход на страницу
        if (link && link.tagName === 'A') {
            // Ничего не делаем, ссылка работает сама по себе
            // Но нужно чтобы клик по заголовку не раскрывал подменю
        ...
        // Клик по пустому месту в заголовке (если кликнули не по ссылке и не по стрелке)
        header.addEventListener('click', function(e) {
            // Если клик был по самому header, а не по ссылке или стрелке
            if (e.target === this || e.target.classList.contains('footer__accordion-title') && e.target.tagName !== 'A') {
                const accordion = this.closest('.footer__accordion');
                if (accordion) {
                    accordion.classList.toggle('active');
                }
...
// ===== ОТОБРАЖЕНИЕ ТОВАРОВ ПО АКЦИИ (СЛАЙДЕР) =====
function renderPromoProducts() {
    ...
    // Инициализируем прогресс-бар для слайдера акций
    setTimeout(function() {
        const slider = document.getElementById('promoSlider');
        const progressFill = document.getElementById('progressFill_promoSlider');
        if (slider && progressFill) {
            ...
            slider.addEventListener('scroll', updatePromoProgress);
            window.addEventListener('resize', updatePromoProgress);
            setTimeout(updatePromoProgress, 300);
        }
    }, 300);
}

// ===== НАВИГАЦИЯ ДЛЯ СЛАЙДЕРОВ АКЦИЙ =====
function initPromoSliderNavigation() {
    const navButtons = document.querySelectorAll('.slider-nav-btn[data-slider="promoSlider"]');
...
// ============================================
// ДУБЛИРУЮЩЕЕ МЕНЮ ПРИ СКРОЛЛЕ (ОБНОВЛЕННОЕ)
// ============================================
(function() {
    ...
    let isVisible = false;
    const offset = 150; // через сколько пикселей показывать
    ...
    window.addEventListener('scroll', function() {
        ...
        if (scrollY > offset && !isVisible) {
            ...
            // Добавляем класс к шапке для тени
            const header = document.querySelector('.header__menu-wrapper');
            if (header) header.classList.add('has-visible-menu');
            isVisible = true;
        } else if (scrollY <= offset && isVisible) {
            navDuplicate.classList.remove('visible');
            const header = document.querySelector('.header__menu-wrapper');
            if (header) header.classList.remove('has-visible-menu');
            isVisible = false;
        }
    });
})();
...
function initSliderProgress() {
    // === 1. СЛАЙДЕРЫ НА СТРАНИЦЕ КАТАЛОГА ===
    ...
    // === 2. СЛАЙДЕР НА ГЛАВНОЙ СТРАНИЦЕ (ЧАСТО ПОКУПАЕМЫЕ ТОВАРЫ) ===
    const mainSlider = document.getElementById('mainSlider');
    if (mainSlider) {
        const progressFill = document.getElementById('progressFill_mainSlider');
        if (progressFill) {
            // Функция обновления прогресса для главной
            ...
            // Обновляем при скролле
            mainSlider.addEventListener('scroll', updateMainProgress);
            
            // Обновляем при изменении размера окна
            window.addEventListener('resize', updateMainProgress);
            
            // Первоначальное обновление с задержкой
            setTimeout(updateMainProgress, 500);
        }
    }
}
...
setTimeout(function() {
    ...
    // Запускаем на странице каталога
    if (currentPage === 'catalog.html' || currentPage === '') {
        setTimeout(initSliderProgress, 500);
    }
    
    // Запускаем на главной странице
    if (currentPage === 'index.html' || currentPage === '') {
        setTimeout(initSliderProgress, 600);
    }
    
    // Запускаем на страницах акций
    if (currentPage.includes('promo-detail')) {
        renderPromoProducts();
        setTimeout(initPromoSliderNavigation, 400);
        // Также запускаем прогресс-бар для слайдера акций
        setTimeout(function() {
            const slider = document.getElementById('promoSlider');
            const progressFill = document.getElementById('progressFill_promoSlider');
            if (slider && progressFill) {
                ...
                slider.addEventListener('scroll', updatePromoProgress);
                window.addEventListener('resize', updatePromoProgress);
                setTimeout(updatePromoProgress, 500);
            }
        }, 500);
    }
    
}, 300);
// ===== ПРОГРЕСС-БАР ДЛЯ ВСЕХ СЛАЙДЕРОВ =====
function initAllSliderProgress() {
    // Находим все слайдеры на странице
    const sliders = document.querySelectorAll('.catalog-slider-container, .catalog-section__grid, .promo-section__grid, .news-section__grid, .advantages-grid');
    
    sliders.forEach(function(slider) {
        // Проверяем, есть ли у слайдера прогресс-бар
        const wrapper = slider.closest('.catalog-slider-wrapper') || slider.closest('.catalog-section') || slider.closest('.promo-section') || slider.closest('.news-section') || slider.closest('.advantages-section');
        if (!wrapper) return;
        
        // Ищем прогресс-бар внутри обертки
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(initAllSliderProgress, 600);
});

// Также запускаем при изменении контента (для динамических слайдеров)
setTimeout(initAllSliderProgress, 1500);
// ===== УПРАВЛЕНИЕ ИСКУССТВЕННОЙ ПОЛОСОЙ ПРОКРУТКИ В НИЖНЕМ МЕНЮ =====
(function() {
    function updateBottomScrollbar() {
        var wrapper = document.querySelector('.bottom-menu__wrapper');
        var thumb = document.getElementById('bottomScrollThumb');
        ...
        if (maxScroll > 0) {
            var percent = (scrollLeft / maxScroll) * 100;
            var width = Math.max(percent, 12);
            thumb.style.width = width + '%';
            console.log('Ширина полосы:', width + '%');
        } else {
    ...
    window.addEventListener('load', function() {
        console.log('Страница загружена, инициализация полосы прокрутки');
        var wrapper = document.querySelector('.bottom-menu__wrapper');
        if (wrapper) {
            wrapper.addEventListener('scroll', updateBottomScrollbar);
            window.addEventListener('resize', updateBottomScrollbar);
            setTimeout(updateBottomScrollbar, 500);
        } else {
            console.log('Контейнер .bottom-menu__wrapper не найден');
        }
    });
    
    // Дополнительный запуск через 2 секунды
    setTimeout(function() {
        console.log('Дополнительная инициализация полосы прокрутки');
        var wrapper = document.querySelector('.bottom-menu__wrapper');
        if (wrapper) {
...
// ============================================
// ИНИЦИАЛИЗАЦИЯ СЛАЙДЕРА НА ГЛАВНОЙ (mainSlider)
// ============================================

// Находим кнопки для mainSlider
const mainSliderPrev = document.querySelector('.slider-nav-btn--prev[data-slider="mainSlider"]');
const mainSliderNext = document.querySelector('.slider-nav-btn--next[data-slider="mainSlider"]');
const mainSliderContainer = document.getElementById('mainSlider');

if (mainSliderPrev && mainSliderNext && mainSliderContainer) {
    // Функция прокрутки
    function scrollMainSlider(direction) {
        const cardWidth = mainSliderContainer.querySelector('.product-card')?.offsetWidth || 280;
        const gap = 15;
        const scrollAmount = (cardWidth + gap) * 3; // Прокручиваем на 3 карточки
        
        if (direction === 'prev') {
            mainSliderContainer.scrollLeft -= scrollAmount;
        } else {
            mainSliderContainer.scrollLeft += scrollAmount;
        }
    }
    ...
    // Обработчики кликов
    mainSliderPrev.addEventListener('click', function(e) {
        e.preventDefault();
        scrollMainSlider('prev');
    });
    
    mainSliderNext.addEventListener('click', function(e) {
        e.preventDefault();
        scrollMainSlider('next');
    });
}

// ============================================
// ОБНОВЛЕНИЕ ПРОГРЕСС-БАРА ДЛЯ mainSlider
// ============================================

function updateMainSliderProgress() {
...
// Добавляем обработчик скролла для mainSlider
if (mainSliderContainer) {
    mainSliderContainer.addEventListener('scroll', updateMainSliderProgress);
    window.addEventListener('resize', updateMainSliderProgress);
    setTimeout(updateMainSliderProgress, 500);
}
...
// ===== ПЛАНШЕТНОЕ МЕНЮ =====
document.addEventListener('DOMContentLoaded', function() {
    const tabletBurger = document.getElementById('tabletBurger');
    const tabletMenu = document.getElementById('tabletMenu');
    const tabletSearchToggle = document.getElementById('tabletSearchToggle');
    const tabletSearch = document.getElementById('tabletSearch');
    const tabletSearchClose = document.getElementById('tabletSearchClose');
    const tabletSearchInput = document.getElementById('tabletSearchInput');
    const tabletSearchResults = document.getElementById('tabletSearchResults');

    // Бургер
    if (tabletBurger && tabletMenu) {
    ...
    // Стрелки подменю
    const tabletArrows = document.querySelectorAll('.tablet-menu__arrow');
    tabletArrows.forEach(function(arrow) {
        arrow.addEventListener('click', function(e) {
            e.stopPropagation();
            const parent = this.closest('.has-submenu');
            if (parent) {
    ...
    // Поиск
    if (tabletSearchToggle && tabletSearch) {
        tabletSearchToggle.addEventListener('click', function(e) {
            e.preventDefault();
            tabletSearch.classList.toggle('active');
            document.body.style.overflow = 'hidden';
            setTimeout(function() {
                if (tabletSearchInput) tabletSearchInput.focus();
            }, 300);
        });
    }

    if (tabletSearchClose) {
        tabletSearchClose.addEventListener('click', function() {
            tabletSearch.classList.remove('active');
            document.body.style.overflow = '';
            if (tabletSearchInput) tabletSearchInput.value = '';
            if (tabletSearchResults) {
                tabletSearchResults.innerHTML = '';
                tabletSearchResults.classList.remove('active');
            }
        });
    }

    tabletSearch.addEventListener('click', function(e) {
        if (e.target === tabletSearch) {
            tabletSearch.classList.remove('active');
            document.body.style.overflow = '';
        }
    });
    ...
    // Поиск товаров
    if (tabletSearchInput) {
        tabletSearchInput.addEventListener('input', function() {
            const query = this.value.trim().toLowerCase();
            if (query.length === 0) {
                if (tabletSearchResults) {
                    tabletSearchResults.innerHTML = '';
            ...
            if (window.allProducts) {
                ...
                try {
                    ...
                    if (typeof waterProducts !== 'undefined') allProducts = allProducts.concat(waterProducts);
                    if (typeof equipmentProducts !== 'undefined') allProducts = allProducts.concat(equipmentProducts);
                    if (typeof accessoryProducts !== 'undefined') allProducts = allProducts.concat(accessoryProducts);
                    if (typeof giftCardProducts !== 'undefined') allProducts = allProducts.concat(giftCardProducts);
                } catch(e) {}
            }
            ...
            if (tabletSearchResults) {
                if (results.length > 0) {
                    tabletSearchResults.innerHTML = results.slice(0, 10).map(function(item) {
                        return '<a href="product-detail.html?id=' + item.id + '" class="tablet-search__result-item">' +
                            '<img src="images/' + item.image + '" alt="' + item.name + '" loading="lazy">' +
                            '<div class="info">' +
                                '<div class="name">' + item.name + '</div>' +
                                '<div class="price">' + (item.price !== null ? item.price + ' ₽' : 'Цена не указана') + '</div>' +
                    ...
                    tabletSearchResults.classList.add('active');
                } else {
                    tabletSearchResults.innerHTML = '<div class="tablet-search__not-found">Ничего не найдено 😕</div>';
                    tabletSearchResults.classList.add('active');
                }
            }
        });
